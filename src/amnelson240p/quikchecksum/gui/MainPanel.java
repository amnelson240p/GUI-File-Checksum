...
public class MainPanel extends JPanel {
	...
	btnVerify.addActionListener(new ActionListener() {
	    public void actionPerformed(ActionEvent e) {
		badChkString = !(checkInputSum());
		repaint(); // clear any updated error flags
		...
		if (isFileSelected && !badChkString) {
		    ...
		    // may not be needed to reflag error
		    badChkString = true;
		    repaint();
		}
	    }
	...
	if (noFileError) {
	    ...
	    g.fillRect(124, 88, 165, 16);
	    g.setColor(Color.RED);
	    g.drawString(NO_FILE_ERROR, 132, 100);
	    noFileError = false;
	}
